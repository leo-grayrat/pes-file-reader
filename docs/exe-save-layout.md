...
- **encHeader 内部结构与 serial 全貌** → 已解，见 §8（encHeader = 192B 随机 + 64B 固定常量 + 64B 随机 tail；serial = UTF-16LE Windows SID）
...
### 8.1 encHeader 320B 明文三段布局

```
encHeader（明文，320B）
┌─────────────┬────────┬───────────────────────────────────────────────┐
│ [0:192]     │ 192B   │ 每存档随机密钥材料：高熵、三档全不同            │
├─────────────┼────────┼───────────────────────────────────────────────┤
│ [192:256]   │ 64B    │ ★全局固定常量：59 样本逐字节全同（见 8.2）    │
├─────────────┼────────┼───────────────────────────────────────────────┤
│ [256:320]   │ 64B    │ 每存档随机 tail = header_key 源（明文透传，     │
│             │        │ 不参与 crypt_header 加密；见 §3/§4）           │
└─────────────┴────────┴───────────────────────────────────────────────┘
```

- `[0:192]` 与 `[256:320]`：三档（及全部 59 样本）逐字节不同、高熵
  （字节种类占比 ~0.55–0.92，H≈5.7–7.4；H 偏低是 64/192B 小样本天花板，
  非结构信号）。纯密钥/随机数材料。
- `[192:256]`：**64 字节全局固定常量**，所有存档逐字节相等：
  ```
...
**验证**：59 样本（BL/EDIT/ML/REPLAY 全覆盖）在 `[192:256]` 逐字节全同，
不同常量值个数 = 1。2⁻⁵¹² 三档巧合可排除。逻辑上：若该常量依赖 `[0:192]` 或
`[256:320]` 的随机字节，则必逐档不同；它全同 ⇒ 必**独立于随机字段**的真·常量。

**来历（未明，已排除多说）**：
| 假说 | 测试 | 结果 |
|---|---|---|
| 主密钥简单变换 | `crypt_stream(mk,0/FF/mk)`、`MT19937(mk)`、`crypt_stream(reverse_longs(mk),…)` | 全不中 |
| 哈希派生 | `SHA512(mk)[:64]`、`SHA512(reverse_longs(mk))`、`SHA512("PES2021")`、`SHA512("")`、`SHA512(mk+"enc")`、`SHA256(mk)` | 全不中 |
| exe 字节省 | 在 `PES2021.exe` 与 `pesXdecrypter/*.exe` 搜该 64B（及前 8B 回退） | 原文 0 命中 |

`third_party/pesXdecrypter/src/crypt.c` 把 `encryptionHeader` 当 **320 字节不透明块**
处理（`cryptHeader` 直接 `cryptStream` 全 320 后 `memcpy` 回 `[256:320]`），
社区/解密器**未标注** `[192:256]` 语义 ⇒ 它是**游戏写档时填的、社区未文档化固定字段**。
未在 exe 原文命中与 SHA-512 K 表被壳混淆同理（壳混淆常量池），故其运行时来源暂不能
由字节签名确认；仅能断定"全局固定、独立于随机密钥材料"。

**是否暗藏主密钥副本**：否。`[0:64]` ≠ `reverse_longs(MASTERKEY)`，整段也不含
`reverse_longs(mk)` 连续 64B，全段无 ≥4 字节连续可打印文本 ⇒ encHeader **不**像
fileHeader 那样再存一份主密钥校验副本（§5 的 mysteryData），`[192:256]` 是另一类未知固定量。
...
固定 `[192:256]` 是 5 个 XOR 项之一，贡献一个**固定偏置**；但 `[0:64]`/`[64:128]`/
`[128:192]`/`[256:320]` 均每存档变 ⇒ **rolling_key 基仍每存档唯一**（实测 BL/EDIT/ML
三档各不同，H≈5.6–5.8）。即该固定常量不破坏每存档密钥唯一性，只是游戏写档格式的
固定结构位。
...
- serial 与 `[192:256]` 固定常量**二者独立**：serial 是 per-install 业务字段（同机同值），
  encHeader 固定常量是 per-build/全局结构字段（所有样本同值）；都"固定"但语义不同。

### 8.5 复现脚本

```bash
python probe_logo.py        # 抽三个 outputs/logo_*.png 并验 IHDR/IDAT 长度
python probe_encheader.py   # encHeader 三段结构、逐字节全局常量检测、rolling_key 基、
                            # 是否暗藏主密钥副本 / 可打印串、BL 全 320B hex dump
# 跨样本常量普适性：BL×4/EDIT/ML×4 已在脚本内，REPLAY×50 可单独 glob 验证（均命中同一常量）
```

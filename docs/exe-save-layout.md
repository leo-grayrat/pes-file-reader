...
| 深拷贝区 | **+0x460 起** | **0xA0 步长 × 14 槽**，每槽调 `0x8e08c0` 深拷贝（子对象 ~0x9D 字节）—— **每槽 = 一名球员的动态记录，ML 成长/训练/状态/合约的最强候选宿主** |
| 尾部 | +0x628 ~ +0x68C | 5×14bit 引用 + 位域 + 多 dword |

> ⚠️ 诚实边界：+0x460 起的 14 槽（+0x460 / +0x500 / +0x5A0 / +0x640 / …，每槽步长 0xA0）
> 使**运行时球队对象实际大于 0x690**（与球员 240B→380B 同理），存档只落 0x690 的
> 序列化部分。这些 0xA0 槽是 squad 成员动态记录的最强候选，但**槽内字段的确切语义
> 仍需逆向"存档 0x690 → 运行时球队对象"的装载路径**才能定案（下轮主攻）。

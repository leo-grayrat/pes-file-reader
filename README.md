...
| 回放结构 | 🔶 部分 | 名单/帧网格/事件区已解；248B blob 非加密/非坐标（姿态码运行时重建） |
| 修改器实操 | 🔶 已定位校验链 | **encHeader[128:192] = SHA-512(data)，加载期比对、不匹配拒绝**（§7）；改 data 须重算该摘要 |
...
- **修改器实操关键**（§7 已闭环）：`encHeader[128:192]` 存 `SHA-512(data 明文)`，
  游戏**加载期逐块比对**、不匹配即拒绝 ⇒ 改 data 后必须**重算该摘要**并更新
  encHeader（`exe/probe_block_hashes.py` 可算）；早前"似乎不校验"备注已作废。
